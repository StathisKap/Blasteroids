...
void spawn_bullet() {
  for (size_t i = 0; i < BULLET_COUNT; i++) // goes through each bullet
  {
    ...
    {
      al_play_sample(global->shoot_sound, 0.3, 0.0, 2.0, ALLEGRO_PLAYMODE_ONCE,
                     NULL);
      global->bullets[i].live = true; // It turns it to true
      global->bullets[i].heading =
          global->ship.heading; // It sets the heading equal to wherever the
                                // ship was looking
      global->bullets[i].sx =
          global->ship.sx +
          10 * sin(global->ship.heading + PI / 2) *
              global->ship
                  .scale; // It sets the starting position to where the ship is
      global->bullets[i].sy =
          global->ship.sy -
          10 * cos(global->ship.heading + PI / 2) * global->ship.scale;
...
void draw_bullet() {
  for (size_t i = 0; i < BULLET_COUNT; i++) {
    if (global->bullets[i].live) {
      ...
      al_draw_filled_circle(global->bullets[i].sx, global->bullets[i].sy,
                            global->bullets[i].size,
                            al_map_rgb(255, 0, 0)); // Draws a red dot
    }
  }
  update_bullet();
}
