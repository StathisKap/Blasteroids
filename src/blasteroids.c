...
int al_register_all() {
  al_register_event_source(global->queue, al_get_timer_event_source(
                                              global->asteroid_rotation_timer));
  al_register_event_source(global->queue,
                           al_get_timer_event_source(global->fire_rate_timer));
...
void blasteroids_init() {
  ...
  global->asteroids =
      malloc(sizeof(Asteroid) * global->asteroids_max_count * 2);
  global->redraw = true;
  global->done = false;
  ...
  al_set_sample_instance_playmode(global->flame_instance,
                                  ALLEGRO_PLAYMODE_LOOP);
  al_attach_sample_instance_to_mixer(global->flame_instance,
...
void draw_score() {
  al_draw_textf(global->font, al_map_rgb(0, 255, 0), DISPLAY_WIDTH / 2.0, 50,
