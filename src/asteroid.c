...
void draw_asteroid() {
  ...
  if (global->AsteroidBitmap == NULL) {
    ...
    al_draw_line(res * -20 + offset, res * 20 + offset, res * -25 + offset,
                 res * 5 + offset, color, thickness);
    al_draw_line(res * -25 + offset, res * 5 + offset, res * -25 + offset,
                 res * -10 + offset, color, thickness);
    al_draw_line(res * -25 + offset, res * -10 + offset, res * -5 + offset,
                 res * -10 + offset, color, thickness);
    al_draw_line(res * -5 + offset, res * -10 + offset, res * -10 + offset,
                 res * -20 + offset, color, thickness);
    al_draw_line(res * -10 + offset, res * -20 + offset, res * 5 + offset,
                 res * -20 + offset, color, thickness);
    al_draw_line(res * 5 + offset, res * -20 + offset, res * 20 + offset,
                 res * -10 + offset, color, thickness);
    al_draw_line(res * 20 + offset, res * -10 + offset, res * 20 + offset,
                 res * -5 + offset, color, thickness);
    al_draw_line(res * 20 + offset, res * -5 + offset, res * 0 + offset,
                 res * 0 + offset, color, thickness);
    al_draw_line(res * 0 + offset, res * 0 + offset, res * 20 + offset,
                 res * 10 + offset, color, thickness);
    al_draw_line(res * 20 + offset, res * 10 + offset, res * 10 + offset,
                 res * 20 + offset, color, thickness);
    al_draw_line(res * 10 + offset, res * 20 + offset, res * 0 + offset,
                 res * 15 + offset, color, thickness);
    al_draw_line(res * 0 + offset, res * 15 + offset, res * -20 + offset,
                 res * 20 + offset, color, thickness);
  ...
  for (short i = 0; i < global->asteroids_max_count * 2; i++) {
    ...
    if (!global->asteroids[i].dead) {
      ...
      al_rotate_transform(&transform,
                          global->asteroids[i].twist); // We rotate it
      al_translate_transform(
          &transform, global->asteroids[i].sx,
          global->asteroids[i].sy); // We move it to an initial position
      al_use_transform(&transform);
      al_draw_tinted_scaled_bitmap(global->asteroids[i].image,
                                   global->asteroids[i].color, 0, 0, size, size,
                                   -s / 2, -s / 2, s, s, 0);
    }
    LOG(2, "Drew Asteroid\n");
  }
...
void Realloc_Asteroid() {
  ...
  global->asteroids = realloc(
      global->asteroids, sizeof(Asteroid) * global->asteroids_max_count * 2);
  if (global->asteroids == NULL) {
    error(
        "Failed to allocate memory to global->asteroids in Realloc Asteroid\n");
    free(temp);
    return;
  }
...
void spawn_asteroid() {
  ...
  LOG(1, "Dead Asteroids %d\tAlive Asteroids %d", dead_asteroids,
  ...
  if (dead_asteroids == global->asteroids_max_count &&
    ...
    // Spawn new asteroids
    for (short i = 0;
         i < global->asteroids_max_count && new_asteroids_count > 0; i++) {
      if (global->asteroids[i].dead) {
        ...
        do {
          // Generate a random value between 0 and 1, and select a value for
          // x_initial based on the result If the random value is 0, set
          // x_initial to a random value between 0 and half the display width
          // minus a fifth of the display width If the random value is 1, set
          // x_initial to the display width minus a random value between 0 and
          // half the display width minus a fifth of the display width
          x_initial =
              rand() % 2 ? (rand() % (DISPLAY_WIDTH / 2 - DISPLAY_WIDTH / 5))
                         : DISPLAY_WIDTH -
                               rand() % (DISPLAY_WIDTH / 2 - DISPLAY_WIDTH / 5);
        } while (
            fabsf(global->ship.sx - x_initial) <
            100); // Repeat the loop if the distance between the ship's x
                  // coordinate and the generated x coordinate is less than 100
        ...
        do {
          // Generate a random value between 0 and 1, and select a value for
          // y_initial based on the result If the random value is 0, set
          // y_initial to a random value between 0 and half the display width
          // minus a fifth of the display width If the random value is 1, set
          // y_initial to the display width minus a random value between 0 and
          // half the display width minus a fifth of the display width
          y_initial =
              rand() % 2 ? (rand() % (DISPLAY_WIDTH / 2 - DISPLAY_WIDTH / 5))
                         : DISPLAY_WIDTH -
            ...
            fabsf(global->ship.sy - y_initial) <
            100); // Repeat the loop if the distance between the ship's x
                  // coordinate and the generated x coordinate is less than 100
        ...
        // Set the asteroid's properties to the generated values and set it as
        // alive
  ...
  for (short i = 0; i < global->asteroids_max_count * 2; i++) {
    if (!global->asteroids[i].dead) {
      ...
      if (global->event.timer.source == global->asteroid_rotation_timer)
        global->asteroids[i].twist +=
            global->asteroids[i].rot_velocity * PI / 180;
      ...
      global->asteroids[i].sx +=
          global->asteroids[i].speed * cos(global->asteroids[i].heading);
      global->asteroids[i].sy +=
          global->asteroids[i].speed * sin(global->asteroids[i].heading);
...
void Split(short j) {
  LOG(1, "Splitting");
  al_play_sample(global->explosion_sound, 0.8, 0.0, 1.0, ALLEGRO_PLAYMODE_ONCE,
                 NULL);

  // Print the status of all asteroids in the array
  for (short i = 0; i < global->asteroids_max_count * 2; i++)
    LOG(1, "Asteroid %d is %s", i,
        global->asteroids[i].dead ? "Dead" : "Alive");
  ...
  LOG(1, "\n\tAsteroid %d is %s", j,
      global->asteroids[j].dead ? "Dead" : "Alive");
  ...
  if (global->asteroids[j].scale > 0.5) {
    ...
    if (global->asteroids_alive + 2 <= global->asteroids_max_count * 2) {
      ...
      for (short i = 0; i < 2; i++) {
        ...
        LOG(1, "Generating Asteroid %d. Max_Count: %d", i,
            global->asteroids_max_count);
        ...
        asteroid1.speed =
            global->asteroids[j].speed * (float)rand() / RAND_MAX * 2.0f;
      ...
      // Handle case where there is not enough space in the array for the new
      // asteroids
