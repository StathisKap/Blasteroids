...
void draw_menu(int *selection, char **options, short options_num) {
  ...
  int total_height =
      font_height *
      4; // 3 options and 1 additional space for between the options

  // The Y position to start drawing the menu options
  int start_y =
      (DISPLAY_HEIGHT - total_height) / 2; // This will center the options
  ...
  for (int i = 0; i < options_num; i++) {
    ...
    al_draw_text(global->font, colors[i], DISPLAY_WIDTH / 2.0,
                 start_y + font_height * i, ALLEGRO_ALIGN_CENTRE, options[i]);
  }
