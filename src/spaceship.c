...
void draw_ship() {
  int xoffset = 20 * global->ship.scale / 2,
      yoffset =
          20 * global->ship.scale / 2; // Half the Bitmap scale * ship scale

  if (global->SpaceShipBitmapCreated == false) {
    float thickness = 2.5f; // just a variable to control the thickness in case
                            // we wanna vary it;
    thickness += global->ship.scale - 1;
    global->ship.image =
        al_create_bitmap(20 * global->ship.scale, 20 * global->ship.scale);
    al_set_target_bitmap(global->ship.image);
    al_draw_line(
        -8 * global->ship.scale + xoffset, 10 * global->ship.scale + yoffset,
        0 * global->ship.scale + xoffset, -10 * global->ship.scale + yoffset,
        global->ship.color, thickness); // We draw the lines
    al_draw_line(
        0 * global->ship.scale + xoffset, -10 * global->ship.scale + yoffset,
        8 * global->ship.scale + xoffset, 10 * global->ship.scale + yoffset,
        global->ship.color, thickness); // We draw the lines
    al_draw_line(
        -6 * global->ship.scale + xoffset, 4 * global->ship.scale + yoffset,
        -1 * global->ship.scale + xoffset, 4 * global->ship.scale + yoffset,
        global->ship.color, thickness); // We draw the lines
    al_draw_line(
        6 * global->ship.scale + xoffset, 4 * global->ship.scale + yoffset,
        1 * global->ship.scale + xoffset, 4 * global->ship.scale + yoffset,
  ...
  if (global->ship.live == true) {
    ...
    al_rotate_transform(&global->ship.transform,
                        global->ship.heading + PI / 2); // We rotate it
    al_translate_transform(
        &global->ship.transform, global->ship.sx,
        global->ship.sy); // We move the transform to where the ship should be
    al_use_transform(&global->ship.transform);
    al_draw_bitmap(global->ship.image, global->ship.scale * -10,
...
void draw_flame() // Draws the flame when it goes forward
{
  int xoffset = 20 * global->ship.scale / 2,
      yoffset =
          20 * global->ship.scale / 2; // Half the Bitmap scale * ship scale
  float thickness = 2.5f; // just a variable to control the thickness in case we
                          // wanna vary it;
  ...
  al_rotate_transform(&global->ship.transform,
                      global->ship.heading + PI / 2); // We rotate it
  al_translate_transform(&global->ship.transform, global->ship.sx,
                         global->ship.sy); // We move it to an initial position
  al_use_transform(&global->ship.transform);
  al_draw_line(0, 25 * global->ship.scale, 5 * global->ship.scale,
               10 * global->ship.scale, al_map_rgb(222, 136, 44),
               thickness); // We draw the lines
  al_draw_line(0, 25 * global->ship.scale, -5 * global->ship.scale,
               10 * global->ship.scale, al_map_rgb(222, 136, 44),
               thickness); // We draw the lines
...
void respawn() {
  if (global->Player_Lives <= 0) {
    gameState = MENU;
    Score score = {"stathis", global->score};
    write_score(&score);
    return;
  }
  ...
  if (al_get_timer_count(global->respawn_timer) % 5) {
    ...
    al_draw_tinted_bitmap(global->ship.image, al_map_rgba_f(0.5, 0.5, 0.5, 0.5),
                          global->ship.scale * -10, global->ship.scale * -10,
                          0);
  }
...
void life_up(int points) {
  ...
  if (global->score >= previous_score + points) {
    ...
    al_play_sample(global->lifeup_sound, 0.7, 0.0, 1.0, ALLEGRO_PLAYMODE_ONCE,
                   NULL);
  }
}
...
void draw_lives() {
  ...
  for (short i = 0; i < global->Player_Lives; i++) {
    al_draw_tinted_scaled_bitmap(
        global->ship.image, al_map_rgba_f(0.2, 0.7, 0.2, 1.0), 0, 0,
        20 * global->ship.scale, 20 * global->ship.scale,
        17 * global->ship.scale * i + 5, // the +5 is the shift from the left
        5,                               // Shift from the top
        15 * global->ship.scale, 15 * global->ship.scale, 0);
  }
}
